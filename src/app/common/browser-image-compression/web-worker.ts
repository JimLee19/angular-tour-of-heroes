/**
 *tsconfig->lib add webworker
 */
import imageCompression from './index';
import compress from './image-compression';

let cnt = 0;
let imageCompressionLibUrl: string;

function createWorker(f: any) {
...
function createSourceObject(str: any) {
  return URL.createObjectURL(new Blob([str], { type: 'application/javascript' }));
}

export function compressOnWebWorker(file: any, options: any) {
